...
export default function CurrencyDetail() {
    ...
    // Значение выбранного периода (в днях)
    ...
    // Функция для запроса данных графика с API с использованием start и end дат
    const fetchChartData = async (days: number) => {
        try {
            const now = new Date()
            // Форматируем дату как dd/mm/yyyy (используем en-GB)
            const endDate = now.toLocaleDateString("en-GB") // текущая дата
            const startDateObj = new Date(now.getTime() - days * 24 * 60 * 60 * 1000)
            const startDate = startDateObj.toLocaleDateString("en-GB") // дата начала периода

            const res = await fetch(
              `https://api.heavenlyweiner.ru/api/currencies/${currencyCode}/history_range?start=${startDate}&end=${endDate}`
            )
            if (!res.ok) {
                throw new Error("Ошибка при получении данных графика")
            }
            // Ожидаем, что API возвращает объект { code: string, history: ChartDataPoint[] }
            const result = await res.json()
            const historyData: ChartDataPoint[] = result.history
            // Сортируем по возрастанию дат: от старой (слева) к текущей (справа)
    ...
    useEffect(() => {
        // При загрузке компонента или изменении кода валюты запрашиваем данные валюты
        fetchCurrencyData()
    }, [currencyCode])
              ...
              <CardContent>
                  <Tabs defaultValue="7" className="w-full" onValueChange={(value) => setTimeRange(value)}>
                      <TabsList className="grid w-full grid-cols-4 bg-gray-800">
                          <TabsTrigger value="7" className="data-[state=active]:bg-gray-700 text-white">7D</TabsTrigger>
                          <TabsTrigger value="30" className="data-[state=active]:bg-gray-700 text-white">30D</TabsTrigger>
                          <TabsTrigger value="90" className="data-[state=active]:bg-gray-700 text-white">90D</TabsTrigger>
                          <TabsTrigger value="365" className="data-[state=active]:bg-gray-700 text-white">1Y</TabsTrigger>
                      </TabsList>
                  </Tabs>
                  <div className="h-[300px] sm:h-[400px] w-full mt-4">
                      <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={chartData}>
                              ...
                              <Tooltip contentStyle={{ backgroundColor: "#1F2937", border: "1px solid #374151" }} labelStyle={{ color: "#9CA3AF" }} formatter={(value: number) => [`${value} ₽`, "Value"]} />
                  ...
                  <CardContent>
                      <div className="space-y-4">
                          <div className="flex justify-between items-center">
                              ...
                              <span className="text-white font-semibold">{currency.statistics.high7d.toFixed(4)}</span>
                          </div>
                          <div className="flex justify-between items-center">
                              <span className="text-gray-400">7d Low</span>
                              <span className="text-white font-semibold">{currency.statistics.low7d.toFixed(4)}</span>
                          </div>
                          <div className="flex justify-between items-center">
